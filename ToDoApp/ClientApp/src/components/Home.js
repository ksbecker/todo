...
export class Home extends Component {
            ...
            <tr key={toDo.id}>
              ...
              <td>
                <ToggleComplete
                  ...
                  onToggleComplete={this.handleDataRefresh}
                />
              </td>
              <td>
                <Delete id={toDo.id} onDelete={this.handleDataRefresh} />
              </td>
            </tr>
          ))}
        </tbody>
  ...
  render() {
    ...
    if (this.state.loading)
      ...
      let incompleteToDosArray = this.state.toDos.filter(
        (toDo) => toDo.completed === false
      );
      incompleteToDos = this.renderToDosTable(incompleteToDosArray);

      let completeToDosArray = this.state.toDos.filter(
    ...
    return (
      <div>
        <Link className="float-right btn btn-primary" to="/add">
          Add
        </Link>
  ...
  async populateToDoData() {
    ...
    this.setState({
      toDos: data.sort(
        (toDo1, toDo2) => Date.parse(toDo1.due) - Date.parse(toDo2.due)
      ),
      loading: false,
    });
  }
